...
hybridSkin.createSideMenu = function () {
  ...
  for (var i = 0; i < inputs.length; i++) {
    var input = inputs[i];
    if (input.type !== "button" || input.fkey == null || input.className.indexOf("hide") >= 0) continue;
  ...
  for (var j = 0; j < windowDivs.length; j++) {
    ...
    for (var k = 0; k < windowInputs.length; k++) {
      ...
      if (windowInput.type === "button" && windowInput.fkey && !dups[windowInput.value]) {
        buttons.push({
          fkey: windowInput.fkey,
          text: windowInput.value
        });
        dups[windowInput.value] = true;
  ...
  if (buttons.length > 0) {
    // Ensure Enter is added first if missing
    if (!gotEnter) {
      buttons.unshift({
        fkey: "Enter",
        text: "Continue"
    ...
    for (let i = 0; i < buttons.length; i++) {
      ...
      fkeyLink.onclick = function (e) {
        ...
        if (target && target.fkey) pressKey(target.fkey);
      };

      panel.appendChild(arrow);
      panel.appendChild(fkeyLink);

      top += fkeyLink.offsetHeight + 8; // add spacing for next link
    }
